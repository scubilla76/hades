...
class TestManifestTest(TempDirTestCase):
    def setup_method(self, method):
        ...
        test_pkg = os.path.join(self.temp_dir, 'config')
        os.mkdir(test_pkg)
        ...
        os.mkdir(static)
        for fname in ['config.js', 'config.js.map', 'config.css', 'config.css.map']:
    ...
    def test_exclude(self):
        """Include everything in config/ except the text files"""
        ml = make_local_path
        self.make_manifest(
            """
            include config/*
        ...
        files = default_files | set([ml('config/c.rst')])
        assert files == self.get_files()

    def test_include_multiple(self):
        """Include with multiple patterns."""
        ml = make_local_path
        self.make_manifest("include config/*.txt config/static/*")
        files = default_files | set([
            ml('config/a.txt'),
            ml('config/b.txt'),
            ml('config/static/config.js'),
            ml('config/static/config.js.map'),
            ml('config/static/config.css'),
            ml('config/static/config.css.map'),
        ])
        assert files == self.get_files()

    def test_graft(self):
        """Include the whole config/static/ directory."""
        ml = make_local_path
        self.make_manifest("graft config/static")
        files = default_files | set([
            ml('config/static/config.js'),
            ml('config/static/config.js.map'),
            ml('config/static/config.css'),
    ...
    def test_graft_global_exclude(self):
        ...
        files = default_files | set([ml('config/static/config.js'), ml('config/static/config.css')])
        assert files == self.get_files()
    ...
    def test_graft_prune(self):
        """Include all files in config/, except for the whole config/static/ dir."""
        ml = make_local_path
        self.make_manifest(
            """
            graft config
        ...
        files = default_files | set([ml('config/a.txt'), ml('config/b.txt'), ml('config/c.rst')])
        assert files == self.get_files()
